...
import {NFTMarket,IERC20,IERC721} from "../src/day15_NFTMarket_v2.sol";
...
contract NFTMarketTest is Test {
    ...
    function test_list_success() public{
        ...
        (address addr,uint price,IERC721 ierc721,uint t)=market.priceList(id);
        assertEq(price,10);
        assertEq(t,tokenId);
        assertEq(addr,saler);
    ...
    function _prepareList() private returns (uint96){
        uint96 tokenId=(uint96)(nftContract.mint(saler,"123"));
        vm.prank(saler);
    ...
    function testFuzz_listAndBuy(uint96 price,address _customer) public{
        ...
        nftContract.approve(address(market),tokenId);
        market.list(IERC721(address(nftContract)),uint96(tokenId),uint96(price));
        uint id=market._ids()-1;
        vm.prank(_customer);
        market.buyNFT(id);
        // 获取NFTInfo结构体并验证其属性
